...
- **Theming**: eight themes (Ember default, Light, Dark, AMOLED, Rosé Pine,
  Dracula, Nord, Catppuccin) are CSS-variable token blocks in
  [theme.css](theme.css), switched by one `data-theme` attribute.
  [theme-boot.js](theme-boot.js) applies the saved theme before first paint;
  swatch pickers live in both the popup and full-page settings.
